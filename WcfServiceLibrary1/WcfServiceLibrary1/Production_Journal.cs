...
namespace WcfServiceLibrary1
{
    ...
    public class Production_Journal : SProduction_Journal
    {
        ...
        {            
            NpgsqlConnection conn = new NpgsqlConnection("Server=localhost;Port=5433;User Id=postgres;Password=qwer;Database=TP_3;");
            NpgsqlCommand command = new NpgsqlCommand();
            string SQL = "UPDATE \"Production_Journal\" SET \"ID_Factory\"=@id_factory, \"ID_Product\"=@id_product, \"Number\"=@id_number WHERE \"ID\"=@id;";
            command.CommandText = SQL;
            string id_factory = GetFactoryID(Factory);
            string id_product = GetProductID(Product);
            command.Parameters.AddWithValue("@factory", id_factory);
        ...
        public string get()
        {
            ...
            NpgsqlConnection conn = new NpgsqlConnection("Server=localhost;Port=5433;User Id=postgres;Password=qwer;Database=TP_3;");
            NpgsqlCommand command = new NpgsqlCommand("SELECT \"Factory\".\"Name\" AS Factory, \"Product\".\"Name\" AS Product, \"Production_Journal\".\"Number\" FROM \"Production_Journal\", \"Factory\", \"Product\" WHERE \"Production_Journal\".\"Is_del\"=0 and \"Production_Journal\".\"ID_Factory\"=\"Factory\".\"ID\" and \"Production_Journal\".\"ID_Product\"=\"Product\".\"ID\"", conn);
            NpgsqlDataReader reader = null;
            try
            {
                conn.Open();
        ...
        public void Delete(string id)
        {
            NpgsqlConnection conn = new NpgsqlConnection("Server=localhost;Port=5433;User Id=postgres;Password=qwer;Database=TP_3;");
            NpgsqlCommand command = new NpgsqlCommand();
            string SQL = "UPDATE \"Production_Journal\" SET \"Is_del\" = true WHERE \"ID\"=@id;";
            command.CommandText = SQL;
            command.Parameters.AddWithValue("@id", id);
            command.Connection = conn;
            NpgsqlDataReader reader = null;
